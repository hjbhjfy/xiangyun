...
import com.rebox.mapper.TeacherInfoMapper;
...
public class TeacherInfoServiceImpl extends ServiceImpl<TeacherInfoMapper, TeacherInfo> implements TeacherInfoService {
    ...
    @Override
    public LoginDTO register(TeacerRegisterQuery registerQuery) {
        Long count = teacherInfoMapper.selectCount(new LambdaQueryWrapper<TeacherInfo>().eq(TeacherInfo::getLoginAct, registerQuery.getLoginAct()));
        if (count > 0) {
            return new LoginDTO(CodeEnum.REGISTER_ACCOUNT_EXIST);
        }
        String encode = passwordEncoder.encode(registerQuery.getLoginPwd());
        TeacherInfo teacherInfo = new TeacherInfo();
        BeanUtils.copyProperties(registerQuery, teacherInfo);
        teacherInfo.setLoginPwd(encode);
    ...
    public LoginDTO login(LoginQuery loginQuery) {
        //根据登录账号查询用户
        TeacherInfo teacherInfo = teacherInfoMapper.selectOne(new LambdaQueryWrapper<TeacherInfo>().eq(TeacherInfo::getLoginAct, loginQuery.getLoginAct()));
        System.out.println(teacherInfo);
        if (ObjectUtils.isEmpty(teacherInfo)) { //用户对象是空，说明用户不存在
            return new LoginDTO(CodeEnum.LOGIN_ACCOUNT_NONE_EXIST);
        }

        //判断密码是否匹配
        if (!passwordEncoder.matches(loginQuery.getLoginPwd(), teacherInfo.getLoginPwd())) { //明文和密文不匹配的时候，那么久返回错误提示
        ...
        BeanUtils.copyProperties(teacherInfo, userDTO);
        ...
        */
        ...
        threadPoolExecutor.execute(() -> {
            teacherInfo.setLastLoginTime(new Date()); //用户最近登录时间
